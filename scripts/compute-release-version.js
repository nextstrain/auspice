...
function computeReleaseVersion({currentVersion, bump, label, branch}) {
  ...
  /* The npm dist-tag decides what a plain `npm install auspice` gets, so prereleases must
   * land somewhere other than "latest". Computed here, rather than in release.yaml, so
   * that the policy is stated once and can be unit tested.
   */
  return {
    version,
    tag: `v${version}`,

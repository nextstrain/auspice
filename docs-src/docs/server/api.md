...
* `type` (optional) - the format of the data returned. Current valid values are "json" (default) or "md". pathname of the requesting page in Auspice. Use this to determine which narrative to return.

**JSON Response (on success):**

The response depends on the `type` specified in the query.

If a JSON is requested (the default) then the narrative file is parsed into JSON format by the server.
The output is generated by the [parseNarrativeFile](server/api.md#parsenarrativefile) function (see below).
...
If a markdown format is requested, then the narrative file is sent to the client unmodified. 

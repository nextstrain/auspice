...
* A whole host of UI updates to improve interaction with Auspice visualisations. These include updated colorings and geographic resolutions, pie charts for discrete variables, a new maintainer, a "zoom reset" buttom, better tree tip info, and a clearer date format!
* If viewing two trees side-by-side in Auspice, you'll now have the ability to define the display of multiple trees and specify the entire pathname for both datasets
* A few new `auspice` commands, including one to help you convert between v1 and v2 JSONs

Read on for rationale behind this update and a detailed explanation of each new feature!


## Backstory

Auspice started as the [nextstrain.org](https://nextstrain.org) visualisation tool, in fact for a long time [nextstrain.org](https://nextstrain.org) _was_ Auspice.
...
While we don't think there is any reason to _not_ upgrade, version 1 will always be available via npm:
```bash
...
For historical reasons, we used to "beautify" certain strings in Auspice - e.g. "usa" became "USA", "new_zealand" became "New Zealand".
...
(For backwards compatability, we've kept the "beautifying" function in the v1-v2 conversion logic so the look of v1 JSONs shouldn't change.)

#### Both metadata and the tree data in a single JSON
The most visible change is probably the reduction of two JSONs (metadata + the tree) into a single JSON.
...
There were also some (minor) bugfixes and improvements to how genes & entropy is rendered.
See [PR #771](https://github.com/nextstrain/auspice/pull/771).

    ...
    case "info":
      ...
      return (
        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 16 16">
          <path
            id="Shape"
            d="M14,1H2C1.5,1,1,1.4,1,2v8c0,0.6,0.5,1,1,1h2v3.5L7.5,11H14c0.6,0,1-0.4,1-1V2C15,1.4,14.6,1,14,1L14,1z"
          />
        </svg>
      );
    case "warning":
...
class Notifications extends React.Component {
  ...
  closeIcon(d) {
    return (
      <div
        onClick={() => {
          this.removeNotificationCallback(d.id);
        }}
  ...
  generateEl(d) {
    return (
      <div key={d.id} className={d.classes.join(" ")}>
        <div className="icon icon-main">{generateIcon(d.notificationType)}</div>
        <div className="content">
          <div className="message item">{d.message}</div>
          <div className="detail item">
            <div className="detail-content">
              {typeof d.details === "string" ? d.details : d.details.map((el) => <div key={el}>{el}</div>)}
            </div>
          </div>
        </div>

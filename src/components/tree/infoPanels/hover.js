...
const ColorBy = ({ node, colorBy, colorByConfidence, colorScale, colorings }) => {
  ...
  /* general case */
  const name = colorings && colorings[colorBy] && colorings[colorBy].title ? colorings[colorBy].title : colorBy;
    ...
    Object.keys(mutationsToDisplay).forEach((prot, idx) => {
      if (idx < nProtsToDisplay) {
        let x =
          prot +
          ":\u00A0\u00A0" +
          mutationsToDisplay[prot].slice(0, Math.min(nDisplay, mutationsToDisplay[prot].length)).join(", ");
...
const VaccineInfo = ({ node, t }) => {
  ...
  if (vaccineInfo.selection_date) {
    renderElements.push(
      <InfoLine name={t("Vaccine selected") + ":"} value={vaccineInfo.selection_date} key="seldate" />
    );
  }
  if (vaccineInfo.start_date) {
    renderElements.push(
      <InfoLine name={t("Vaccine start date") + ":"} value={vaccineInfo.start_date} key="startdate" />
    );
  }
...
const HoverInfoPanel = ({ hovered, colorBy, colorByConfidence, colorScale, panelDims, colorings, t }) => {

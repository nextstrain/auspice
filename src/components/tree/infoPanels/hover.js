...
const StrainName = ({ name }) => <div style={infoPanelStyles.tooltipHeading}>{name}</div>;
...
const BranchLength = ({ node, t }) => {
  ...
  if (divergence) {
    elements.push(
      <InfoLine name={t("Divergence") + ":"} value={formatDivergence(divergence)} key="div" />
    );
  }
  ...
  if (numDate !== undefined) {
    ...
    if (numDateConfidence && numDateConfidence[0] !== numDateConfidence[1]) {
      ...
      const dateRange = [
        numericToCalendar(numDateConfidence[0]),
        numericToCalendar(numDateConfidence[1])
      ];
      if (dateRange[0] !== dateRange[1]) {
        elements.push(
          <InfoLine
            name={t("Date Confidence Interval") + ":"}
...
const ColorBy = ({ node, colorBy, colorByConfidence, colorScale, colorings }) => {
  ...
  /* general case */
  const name =
    colorings && colorings[colorBy] && colorings[colorBy].title
  ...
  const showCurrentColorByWithoutConfidence = () => {
    ...
    return isValueValid(value) ? <InfoLine name={`${name}:`} value={value} /> : null;
  };
  ...
  if (colorByConfidence === true) {
    ...
    if (
      !node.hasChildren &&
      Object.keys(confidenceData).length === 1 &&
      Object.values(confidenceData)[0] > 0.99
    ) {
    ...
    Object.keys(mutationsToDisplay).forEach((prot, idx) => {
      if (idx < nProtsToDisplay) {
        let x =
          prot +
          ":\u00A0\u00A0" +
          mutationsToDisplay[prot]
            .slice(0, Math.min(nDisplay, mutationsToDisplay[prot].length))
            .join(", ");
...
const VaccineInfo = ({ node, t }) => {
  ...
  if (vaccineInfo.selection_date) {
    renderElements.push(
      <InfoLine
        name={t("Vaccine selected") + ":"}
        value={vaccineInfo.selection_date}
        key="seldate"
      />
    );
  }
  if (vaccineInfo.start_date) {
    renderElements.push(
      <InfoLine
        name={t("Vaccine start date") + ":"}
        value={vaccineInfo.start_date}
        key="startdate"
      />
    );
  }
  if (vaccineInfo.end_date) {
    renderElements.push(
      <InfoLine name={t("Vaccine end date") + ":"} value={vaccineInfo.end_date} key="enddate" />
    );
  }
...
const Container = ({ node, panelDims, children }) => {
  ...
  const xPos =
    node.shell.that.params.orientation[0] === -1
...
const Comment = ({ children }) => <div style={infoPanelStyles.comment}>{children}</div>;
    ...
    <Container node={node} panelDims={panelDims}>
      {hovered.type === ".tip" ? (
        ...
        <>
          ...
          />
          <Comment>
            {idxOfInViewRootNode === node.arrayIdx
              ? t("Click to zoom out to parent clade")
              : t("Click to zoom into clade")}
          </Comment>
        </>
      )}
    </Container>

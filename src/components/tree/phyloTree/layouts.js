...
export const timeVsRootToTip = function timeVsRootToTip() {
  ...
  const secondMomentTime =
    sum(this.nodes.filter((d) => d.terminal).map((d) => (d.depth - offset) * (d.depth - offset))) /
    nTips;
...
export const unrootedLayout = function unrootedLayout() {
  ...
  this.nodes.forEach((d) => {
    d.branchLength = d.depth - d.pDepth;
  });
...
export const setScales = function setScales(margins) {
  ...
  if (this.layout === "radial" || this.layout === "unrooted") {
    ...
    this.xScale.range([
      0.5 * xSlack + margins["left"] || 0,
      width - 0.5 * xSlack - (margins["right"] || 0)
    ]);
    this.yScale.range([
      0.5 * ySlack + margins["top"] || 0,
      height - 0.5 * ySlack - (margins["bottom"] || 0)
    ]);
  } else {
...
export const mapToScreen = function mapToScreen() {
  ...
  if (this.vaccines) {
    this.vaccines.forEach((d) => {
      ...
      d.vaccineCross = ` M ${xTipCross - n},${yTipCross - n} L ${xTipCross + n},${yTipCross +
        n} M ${xTipCross - n},${yTipCross + n} L ${xTipCross + n},${yTipCross - n}`;
    });
  }
  ...
  if (this.layout === "clock" || this.layout === "unrooted") {
    ...
    this.nodes.forEach((d) => {
      ...
      if (this.params.confidence) {
        d.confLine = ` M ${this.xScale(d.conf[0])},${d.yBase} L ${this.xScale(d.conf[1])},${
          d.yTip
        }`;
      }
    });
    ...
    this.nodes.forEach((d) => {
      d.cBarStart = this.yScale(d.yRange[0]);
    });
    this.nodes.forEach((d) => {
      d.cBarEnd = this.yScale(d.yRange[1]);
    });
    this.nodes.forEach((d, i) => {
      d.branch = [
        " M " +
          (d.xBase - stem_offset_radial[i] * Math.sin(d.angle)).toString() +
          " " +
      ...
      if (!d.terminal) {
        d.branch[1] = [
          " M " +
            this.xScale(d.xCBarStart).toString() +
            " " +
            this.yScale(d.yCBarStart).toString() +
            " A " +
            (this.xScale(d.depth) - this.xScale(offset)).toString() +
            " " +
            ...
            " 1 " +
            " " +
            this.xScale(d.xCBarEnd).toString() +
            "," +
            this.yScale(d.yCBarEnd).toString()
        ];
      }
    });
  }

...
export const timeVsRootToTip = function timeVsRootToTip() {
  ...
  const secondMomentTime =
    sum(this.nodes.filter((d) => d.terminal).map((d) => (d.depth - offset) * (d.depth - offset))) / nTips;
...
export const mapToScreen = function mapToScreen() {
  ...
  if (this.vaccines) {
    this.vaccines.forEach((d) => {
      ...
      d.vaccineCross = ` M ${xTipCross - n},${yTipCross - n} L ${xTipCross + n},${yTipCross + n} M ${xTipCross - n},${
        yTipCross + n
      } L ${xTipCross + n},${yTipCross - n}`;
    });
  }

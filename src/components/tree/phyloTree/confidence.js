...
export const removeConfidence = function removeConfidence(dt) {
  ...
  if (dt) {
    this.groups.confidenceIntervals.selectAll("*").transition().duration(dt).style("opacity", 0).remove();
  } else {
...
export const drawConfidence = function drawConfidence(dt) {
  ...
  if (dt) {
    ...
    this.groups.confidenceIntervals.transition().duration(dt).style("opacity", 0.5);
  } else {
    this.groups.confidenceIntervals
      .selectAll(".conf")
      .data(this.nodes)
...
export const calcConfidenceWidth = (el) =>
  el["stroke-width"] === 1 ? 0 : el["stroke-width"] > 6 ? el["stroke-width"] + 6 : el["stroke-width"] * 2;

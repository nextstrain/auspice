...
export const onTipClick = function onTipClick(d) {
  ...
  const tipSelected =
    d.that.params.orientation[0] === 1 ? { treeIdx: d.n.arrayIdx } : { treeTooIdx: d.n.arrayIdx };
  this.props.dispatch(
...
export const onBranchClick = function onBranchClick(d) {
  ...
  if (legalBranchLabels && legalBranchLabels.length) {
    ...
    legalBranchLabels.sort(
      (a, b) => availableBranchLabels.indexOf(a) - availableBranchLabels.indexOf(b)
    );
    // then use the first!

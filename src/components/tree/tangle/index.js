...
const makeTipPathGenerator = (props) => (idxs) => {
  ...
  return `M ${tip1.xTip},${tip1.yTip} H ${props.width / 2 -
    props.spaceBetweenTrees / 2} L ${props.width / 2 + props.spaceBetweenTrees / 2},${
    tip2.yTip
  } H ${props.width / 2 + props.spaceBetweenTrees / 2 + tip2.xTip}`;
};
...
class Tangle extends React.Component {
  ...
  render() {
    ...
    const lefts = [
      this.props.width / 2 - this.props.spaceBetweenTrees / 2,
      this.props.width / 2 + this.props.spaceBetweenTrees / 2
    ];
    return (
      <div id="TangleContainer">
        <div
          id="MainTreeTitle"
          style={{ ...textStyles, left: lefts[0], transform: "translateX(-100%)" }}
        >

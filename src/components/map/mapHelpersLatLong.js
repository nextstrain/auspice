 ...
 */
const createOrUpdateArcs = (
  visibleNodes,
  legendValues,
  colorBy,
  nodeColors,
  currentArcs = undefined
) => {
  const colorByIsGenotype = isColorByGenotype(colorBy);
  const legendValueToArcIdx = {};
  const undefinedArcIdx =
    legendValues.length; /* the arc which is grey to represent undefined values on tips */
  let arcs;
  if (currentArcs) {
...
const maybeConstructTransmissionEvent = (
  ...
  if (validLatLongPair) {
    ...
    const interpolator = interpolateNumber(
      getTraitFromNode(node, "num_date"),
      getTraitFromNode(child, "num_date")
    );
  ...
  nodes.forEach((node) => {
    ...
    node.children.forEach((child) => {
      ...
      try {
        ...
        console.warn(
          `Error trying to access ${id} in transmissionIndices. Map transmissions may be wrong.`
        );
      }
    });
  });

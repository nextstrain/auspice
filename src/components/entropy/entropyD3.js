...
EntropyChart.prototype._getZoomCoordinates = function _getZoomCoordinates(parsed, geneMap) {
  ...
  return [Math.max(startEnd[0] - multiplier, 0), Math.min(startEnd[1] + multiplier, this.scales.xNav.domain()[1])];
};
...
EntropyChart.prototype._drawZoomGenes = function _drawZoomGenes(annotations) {
  ...
  const strokeCol =
    posInSequence < 1e6 ? "white" : "black"; /* black for large because otherwise disappear against background */
  ...
  const selection = this.geneGraph.selectAll(".gene").data(visibleAnnots).enter().append("g");
  selection
    .append("rect")
    .attr("class", "gene")
    .attr("x", (d) => startG(d))
  ...
  selection
    .append("text")
    .attr("x", (d) => this.scales.xMain(d.start) + (this.scales.xMain(d.end) - this.scales.xMain(d.start)) / 2)
...
EntropyChart.prototype._drawGenes = function _drawGenes(annotations) {
  ...
  const selection = this.navGraph.selectAll(".gene").data(annotations).enter().append("g");
  selection
    .append("rect")
    .attr("class", "gene")
  ...
  selection
    .append("text")
    .attr("x", (d) => this.scales.xNav(d.start) + (this.scales.xNav(d.end) - this.scales.xNav(d.start)) / 2)
    .attr("y", (d) => readingFrameOffset(d.strand) + 5)
    .attr("dy", ".7em")
    .attr("text-anchor", "middle")
    .style("fill", () => "white")
...
EntropyChart.prototype._highlightSelectedBars = function _highlightSelectedBars() {
  for (const d of this.selectedNodes) {
    if (this.aa && !d.prot)
      return; /* if we've switched from NT to AA by selecting a gene, don't try to highlight NT position! */
    const id = this.aa ? `#prot${d.prot}${d.codon}` : `#nt${d.x}`;
    const fillVal = this.aa ? this.geneMap[d.prot].fill : d.prot ? this.geneMap[d.prot].fill : "red";
...
EntropyChart.prototype._drawBars = function _drawBars() {
  ...
  const chart = this.mainGraph.append("g").attr("clip-path", "url(#clip)").selectAll(".bar");
...
EntropyChart.prototype._setScales = function _setScales(xMax, yMax) {
  ...
  this.scales.xMin = 0;
  this.scales.xMain = scaleLinear().domain([0, xMax]).range([this.offsets.x1, this.offsets.x2]);
  this.scales.xNav = scaleLinear().domain([0, xMax]).range([this.offsets.x1, this.offsets.x2]);
...
EntropyChart.prototype._addBrush = function _addBrush() {
  ...
  this.brush = brushX()
    ...
    .extent([
      [this.offsets.x1, 0],
      [this.offsets.width + 20, this.offsets.heightNav - 1 + 25]
    ])
    .on("brush", () => {

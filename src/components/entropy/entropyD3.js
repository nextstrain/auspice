...
EntropyChart.prototype.render = function render(props) {
  ...
  this.zoomCoordinates = isColorByGenotype(props.colorBy)
    ? this._getZoomCoordinates(
      decodeColorByGenotype(props.colorBy, props.geneLength),
      props.geneMap
    )
  ...
  if (gene !== undefined && start !== undefined && end !== undefined) {
    ...
    const multiplier =
      gene === nucleotide_gene
        ? 0
        : 1 * geneLength; /* scale genes to decent size, don't scale nucs */
    this.navGraph.select(".brush").call(this.brush.move, () => {
      /* scale so genes are a decent size. stop brushes going off graph */
      return [
        Math.max(this.scales.xNav(start - multiplier), this.scales.xNav(0)),
...
EntropyChart.prototype._getZoomCoordinates = function _getZoomCoordinates(parsed, geneMap) {
  ...
  return [
    Math.max(startEnd[0] - multiplier, 0),
...
EntropyChart.prototype._drawZoomGenes = function _drawZoomGenes(annotations) {
  ...
  const strokeCol =
    posInSequence < 1e6
      ? "white"
      : "black"; /* black for large because otherwise disappear against background */
  ...
  const visibleAnnots = annotations.filter(
    (annot /* try to prevent drawing genes if not visible */) =>
      (annot.start < this.scales.xMain.domain()[1] &&
        annot.start > this.scales.xMain.domain()[0]) ||
      (annot.end > this.scales.xMain.domain()[0] && annot.end < this.scales.xMain.domain()[1]) ||
      (annot.start <= this.scales.xMain.domain()[0] && annot.end >= this.scales.xMain.domain()[1])
  ); // for extreme zoom, keep plotting if both ends off graph!
  /* stop gene plots from extending beyond axis if zoomed in */
  const startG = (d) =>
    d.start > this.scales.xMain.domain()[0] ? this.scales.xMain(d.start) : this.offsets.x1;
  const endG = (d) =>
    d.end < this.scales.xMain.domain()[1] ? this.scales.xMain(d.end) : this.offsets.x2;
  const selection = this.geneGraph
    .selectAll(".gene")
    .data(visibleAnnots)
    .enter()
    .append("g");
  ...
  selection
    ...
    .attr(
      "x",
      (d) => this.scales.xMain(d.start) + (this.scales.xMain(d.end) - this.scales.xMain(d.start)) / 2
    )
    ...
    .style("fill", () => "white")
    .text(
      (d) => (endG(d) - startG(d) > 15 ? d.prot : "")
    ); /* only print labels if gene large enough to see */
};
...
EntropyChart.prototype._drawGenes = function _drawGenes(annotations) {
  ...
  const startG = (d) =>
    d.start > this.scales.xNav.domain()[0] ? this.scales.xNav(d.start) : this.offsets.x1;
  const endG = (d) =>
    d.end < this.scales.xNav.domain()[1] ? this.scales.xNav(d.end) : this.offsets.x2;
  const selection = this.navGraph
    .selectAll(".gene")
    .data(annotations)
    .enter()
    .append("g");
  ...
  selection
    ...
    .attr(
      "x",
      (d) => this.scales.xNav(d.start) + (this.scales.xNav(d.end) - this.scales.xNav(d.start)) / 2
    )
    .attr("y", (d) => readingFrameOffset(d.strand) + 5)
    .attr("dy", ".7em")
    .attr("text-anchor", "middle")
...
EntropyChart.prototype._drawBars = function _drawBars() {
  ...
  if (this.aa) {
    if (posInView > 600) {
      ...
      barWidth = (d) =>
        this.scales.xMain(this._aaToNtCoord(d.prot, d.codon) + 2.6) -
        this.scales.xMain(this._aaToNtCoord(d.prot, d.codon));
    }
  } else if (posInView > 1000) {
...
EntropyChart.prototype._drawAxes = function _drawAxes() {
  ...
  if (visPos > 1e6) {
    /* axes number differently if large genome */
...
EntropyChart.prototype._addBrush = function _addBrush() {
  this.brushed = function brushed() {
    ...
    if (!d3event.selection) {
      /* This keeps brush working if user clicks (zoom out entirely) rather than click-drag! */
      this.navGraph.select(".brush").call(this.brush.move, () => {
  ...
  this.brushFinished = function brushFinished() {
    ...
    if (
      d3event.sourceEvent instanceof MouseEvent &&
      (!d3event.selection ||
    ...
    if (this.brushHandle) {
      this.brushHandle
        ...
        .attr(
          "transform",
          (d, i) =>
            "translate(" + this.scales.xNav(s[i]) + "," + (this.offsets.heightNav + 25) + ")"
        );
    }
  };
  ...
  this.brushHandle = this.gBrush
    ...
    .attr(
      ...
      (d) =>
        d.type === "e"
          ? "translate(" +
            (this.scales.xNav(this.zoomCoordinates[1]) - 1) +
            "," +
            (this.offsets.heightNav + 25) +
            ")"
          : "translate(" +
            (this.scales.xNav(this.zoomCoordinates[0]) + 1) +
            "," +
            (this.offsets.heightNav + 25) +
            ")"
...
EntropyChart.prototype._createZoomFn = function _createZoomFn() {
  return function zoomed() {
    ...
    const tempZoomCoordinates = [
      Math.max(this.zoomCoordinates[0] + amountZoomChange, this.scales.xNav(0)),
    ...
    this.zoomCoordinates = tempZoomCoordinates; // does this do anything?? Unsure.

    /* rescale the x axis (not y) */ t.rescaleX(this.scales.xMain);
    this.axes.xMain = this.axes.xMain.scale(this.scales.xMain);
    this.svg.select(".xMain.axis").call(this.axes.xMain);
    this._drawBars();
    this._drawZoomGenes(this.annotations);
...
EntropyChart.prototype._drawMainNavElements = function _drawMainNavElements() {
  this.mainGraph = this.svg
    .append("g")
    .attr("class", "main")
    .attr("transform", "translate(" + this.offsets.x1 + "," + this.offsets.y1Main + ")");
  this.navGraph = this.svg
    .append("g")
    .attr("class", "nav")
    .attr("transform", "translate(" + this.offsets.x1 + "," + this.offsets.y1Nav + ")");
  this.geneGraph = this.svg
    .append("g")
    .attr("class", "Gene")

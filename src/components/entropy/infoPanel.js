  ...
  return (
    <div style={styles.container}>
      <div className={"tooltip"} style={infoPanelStyles.tooltip}>
        <div>
          {mutType === "aa"
            ? t("Codon {{codon}} in protein {{protein}}", { codon: hovered.d.codon, protein: hovered.d.prot })
            : hovered.d.prot
            ? `${t("Nucleotide {{nuc}}", { nuc: hovered.d.x })} (${t("Codon {{codon}} in protein {{protein}}", {
                codon: codonFromNuc,
        ...
        <p />
        <div>{mutType === "aa" ? t("Nuc positions {{a}} to {{b}}", { a: nucPos - 2, b: nucPos }) : ``}</div>
        <p />
        <div>{isNegStrand === null ? `` : isNegStrand ? t("Negative strand") : t("Positive strand")}</div>
        <p />
        <div>{showCounts ? `${t("Num mutations")}: ${hovered.d.y}` : `${t("entropy")}: ${hovered.d.y}`}</div>

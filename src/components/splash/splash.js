...
const SplashContent = ({ available, browserDimensions, dispatch, errorMessage, changePage }) => {
  ...
  const ErrorMessage = () => (
    <CenterContent>
      <div>
        ...
        <p style={{ color: "rgb(222, 60, 38)", fontWeight: 400, fontSize: "18px" }}>{`Details: ${errorMessage}`}</p>
  ...
  const ListAvailable = ({ type, data }) => (
    <>
      <div style={{ fontSize: "26px" }}>{`Available ${type}:`}</div>
      {data ? (
  ...
  const Footer = () => <CenterContent>{logos}</CenterContent>;

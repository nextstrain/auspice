...
class Info extends React.Component {
    ...
    buttons.push(
      <div key={"timefilter"} style={{ display: "inline-block" }}>
        <div
          ...
          onClick={() => {
            this.props.dispatch(
              changeDateFilter({ newMin: this.props.absoluteDateMin, newMax: this.props.absoluteDateMax })
            );
          }}
          role="button"
          tabIndex={0}
  ...
  addNonAuthorFilterButton(buttons, filterName) {
    this.props.filters[filterName].sort().forEach((itemName) => {
      ...
      buttons.push(
        displayFilterValueAsButton(this.props.dispatch, this.props.filters, filterName, itemName, display, true)
      );
    });
  }

...
function setUpSimulation(demes: DemeForWorker[], transmissionCounts: TransmissionCounts, svgWidth: number, svgHeight: number): void {
  ...
  simulation
    ...
    .force("repel", // generally move demes away from each other. TODO XXX: disable this if transmissions present?
      forceManyBody()
        .strength(-15)
    );
  ...
  if (transmissionCounts.size) {
    const links = _summariseLinks(demes, transmissionCounts);
    const maxTransmissions = links.reduce((acc, link) => link.nTransmissions>acc ? link.nTransmissions : acc, 0);
    const dist = min([svgWidth, svgHeight]);
    const zmin = dist/10;
    const zmax = dist;
    const idealDistance = (link: Link): number => {
      return link.radii + zmin + Math.exp(-7.5*(link.nTransmissions+1)/maxTransmissions)*(zmax-zmin)
    }
...
interface Link {
  /* one deme involved in the link. Note: this may be the "source" or "destination" - source is d3's term */
  source: number
  /* one deme involved in the link. Note: this may be the "source" or "destination" - target is d3's term */
  target: number
  /* sum of each deme's radius */
  radii: number // TODO XXX -- this should be max radius, right?
  /* number of transmissions from source -> target + target -> source */
  nTransmissions: number
}

function _summariseLinks(demes: DemeForWorker[], transmissionCounts: TransmissionCounts): Link[] {
  const links: Link[] = [];

...
class DateRangeInputs extends React.Component {
  ...
  updateFromSlider(debounce, numDateValues) {
    ...
    const newRange = {
      min: numericToCalendar(numDateValues[0]),
    ...
    if (this.props.dateMin !== newRange.min && this.props.dateMax === newRange.max) {
      ...
      // update both
      this.props.dispatch(
        changeDateFilter({ newMin: newRange.min, newMax: newRange.max, quickdraw: debounce })
      );
      ...
      <div>
        <div style={{ width: controlsWidth }}>
          <Slider // numDates are handed to Slider
            ...
            minDistance={
              minDistanceDateSlider *
              (this.props.absoluteDateMaxNumeric - this.props.absoluteDateMinNumeric)
            }
            pearling
            withBars
          />

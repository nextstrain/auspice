...
const createLegendBounds = (legendValues) => {
  ...
  legendBounds[legendValues[len - 1]] = [
    valBetween(legendValues[len - 2], legendValues[len - 1]),
    10000
  ];
  return legendBounds;
};
    ...
    if (colorings[colorBy].scale) {
      ...
      if (!Array.isArray(colorings[colorBy].scale)) {
        ...
        if (extraVals.length) {
          ...
          range = range.concat(
            createListOfColors(extraVals.length, [rgb(192, 192, 192), rgb(32, 32, 32)])
          );
        }
        colorScale = scaleOrdinal()
          .domain(domain)
      ...
      if (allInteger) {
        ...
        console.warn(
          "Using a categorical scale as currently ordinal scales must only contain integers"
        );
        continuous = false;
        colorScale = createDiscreteScale(legendValues, "categorical");
      }
      ...
      switch (colorBy) {
        case "num_date":
          ...
          if (treeTooNodes) {
            ...
            vals.concat(
              treeTooNodes.filter((n) => !n.hasChildren).map((n) => getTraitFromNode(n, "num_date"))
            );
          }
          vals = vals.sort();
          domain = [rootDate];

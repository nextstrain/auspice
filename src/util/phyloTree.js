...
PhyloTree.prototype.mapToScreen = function() {
  this.setScales(this.options.margins || {
    left: 50,
    right: 50,
    top: 50,
    bottom: 50
  });
  const tmp_xValues = this.nodes.map(function(d) {
    return d.x
  });
  const tmp_yValues = this.nodes.map(function(d) {
    return d.y
  });
  if (this.layout === "radial") {
    const maxSpan = d3.max([-d3.min(tmp_xValues), d3.max(tmp_xValues), -d3.min(tmp_yValues), d3.max(tmp_yValues)]);
  ...
  this.nodes.forEach(function(d) {
    d.xTip = tmp_xScale(d.x)
  });
  this.nodes.forEach(function(d) {
    d.yTip = tmp_yScale(d.y)
  });
  this.nodes.forEach(function(d) {
    d.xBase = tmp_xScale(d.px)
  });
  this.nodes.forEach(function(d) {
    d.yBase = tmp_yScale(d.py)
  });
  ...
  if (this.layout === "rootToTip") {
    ...
    this.nodes.forEach(function(d) {
      d.cBarStart = tmp_yScale(d.yRange[0])
    });
    this.nodes.forEach(function(d) {
      d.cBarEnd = tmp_yScale(d.yRange[1])
    });
    this.nodes.forEach(function(d) {
      d.branch = " M " + d.xBase.toString() + "," + d.yBase.toString() +
        " L " + d.xTip.toString() + "," + d.yTip.toString() +
...
PhyloTree.prototype.addGrid = function(layout) {
  ...
  this.gridColor = "rgb(220,220,220)";

  const xmin = (this.xScale.domain()[0] > 0) ? this.xScale.domain()[0] : 0.0;
  const xmax = layout == "radial" ?
    d3.max([this.xScale.domain()[1], this.yScale.domain()[1], -this.xScale.domain()[0], -this.yScale.domain()[0]]) :
...
PhyloTree.prototype.updateSelectedBranchOrTip = function (oldSelected, newSelected) {
  ...
  console.log(oldSelected.d, newSelected.d)
  this.nodes.forEach((d, i) => {
    d.update = false;
    if (
      d.branch === oldSelected.d.branch ||
      d.branch === newSelected.d.branch ||
      d.n.attr.strain === oldSelected.d.n.attr.strain ||
  ...
  this.svg.selectAll(".tip").filter(function(d) {
    ...
    .style("stroke", function(d) {
      return d.n.attr.strain === newSelected.d.n.attr.strain && newSelected.type === ".tip" ? d.fill : "none";
    })
    .style("stroke-dasharray", function(d) {
      return d.n.attr.strain === newSelected.d.n.attr.strain && newSelected.type === ".tip" ? "2, 2" : "none";
    })
    .style("fill", function(d) {
      return d.n.attr.strain === newSelected.d.n.attr.strain && newSelected.type === ".tip" ? "white" : d.fill;
    });
};

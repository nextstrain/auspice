...
const determineLegendMatch = (selectedLegendItem, node, colorScale) => {
  ...
  if (colorScale.continuous) {
    return (
      nodeAttr <= colorScale.legendBounds[selectedLegendItem][1] &&
...
export const calcTipRadii = ({
  ...
  if (selectedLegendItem !== false && tree && tree.nodes) {
    return tree.nodes.map(
      (d) =>
        determineLegendMatch(selectedLegendItem, d, colorScale) ? tipRadiusOnLegendMatch : tipRadius
    );
  } else if (geoFilter.length === 2 && tree && tree.nodes) {
    return tree.nodes.map(
      (d) => (determineLocationMatch(d, ...geoFilter) ? tipRadiusOnLegendMatch : tipRadius)
    );
  } else if (searchNodes) {
    return tree.nodes.map(
      (d) => (d.name.toLowerCase().includes(searchNodes) ? tipRadiusOnLegendMatch : tipRadius)
    );

...
const handleMetadata = async (dispatch, getState, file) => {
  ...
  if (strainsToProcess.size === 0 || newColorByNames.length === 0) {
    return dispatch(
      errorNotification({
        ...
        details:
          newColorByNames.length === 0
            ? "No columns to add as colorings"
            : "No taxa which match those in the tree"
      })
    );
  }
  ...
  if (taxaInCsvButNotInTree.length) {
    ...
    console.warn(
      "Ignoring these taxa from the CSV as they don't appear in the tree:",
      taxaInCsvButNotInTree
    );
  }
  if (colorBysIgnored.length) {
    dispatch(
      warningNotification({
        message: `Ignoring ${
          colorBysIgnored.length
        } CSV fields as they are already set as colorings or are "special" cases to be ignored`,
  ...
  return dispatch(
    successNotification({
      ...
      details: `${newColorByNames.length} new field${newColorByNames.length > 1 ? "s" : ""} for ${
        strainsToProcess.size
      } node${strainsToProcess.size > 1 ? "s" : ""}`
    })
  );
};

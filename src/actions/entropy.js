...
export const updateEntropyVisibility = debounce(
  (dispatch, getState) => {
    const { entropy, controls, tree } = getState();
    if (!tree.nodes || !tree.visibility || !entropy.geneMap || controls.animationPlayPauseButton !== "Play") {
      return;
    }
    const [data, maxYVal] = calcEntropyInView(
      tree.nodes,
      tree.visibility,
      controls.mutType,
      entropy.geneMap,

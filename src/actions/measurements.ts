...
export const changeMeasurementsCollection = (
  ...
  batch(() => {
    ...
    /* After the collection has been updated, check if we need to update the measurement coloring data */
...
export const changeMeasurementsGroupBy = (
  ...
  const { controls, measurements } = getState();
  /* If the measurementsColorGrouping is no longer valid, then set to undefined to remove it */
  const groupingValues = measurements.collectionToDisplay.groupings.get(newGroupBy).values || [];
  const measurementsColorGrouping = groupingValues.includes(controls.measurementsColorGrouping)
    ? controls.measurementsColorGrouping
    : undefined;
  ...
  batch(() => {
    ...
    /* After the group by has been updated, check if we need to update the measurement coloring data */
    updateMeasurementsColorData(
      newControls.measurementsColorGrouping,
      controls.measurementsColorGrouping,
      controls.colorBy,
...
function createMeasurementNodeAttrs(
  filters: MeasurementFilters,
  groupBy: string,
  groupingValue: string,
  collection: Collection,
): {
  [strain: string]: {[measurementsColorBy: string]: {value: number}}
} {
...
function createMeasurementsColoring (
  groupingValue: string,
  collection: Collection
): {
  [key: string]: {
...
const addMeasurementsColorData = (
  ...
  const { controls, measurements } = getState();
  const newNodeAttrs = createMeasurementNodeAttrs(
    controls.measurementsFilters,
    controls.measurementsGroupBy,
    groupingValue,
    measurements.collectionToDisplay,
  );
  const newColorings = createMeasurementsColoring(
    groupingValue,
    measurements.collectionToDisplay,
  )
...
export const combineMeasurementsControlsAndQuery = (
  ...
  updatedQuery: Query,
  updatedMetadata: { colorings?: {} } // TODO: replace with Partial<Metadata>
} => {
  ...
  if (typeof(updatedQuery.c) === 'string' && isMeasurementColorBy(updatedQuery.c)) {
    ...
    // otherwise save the colorGrouping to be able to add measurements metadata and color scale
    if (!groupingValues.includes(colorGrouping)) {
      ...
      // Replicating the state changes applied for `ADD_EXTRA_METADATA`
      collectionControls['coloringsPresentOnTree'] = updatedQuery.c;
      updatedMetadata['colorings'] =
    }
  }
